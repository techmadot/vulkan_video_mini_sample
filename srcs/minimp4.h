...
#define MINIMP4_TRANSCODE_SPS_ID  01
...
#define TRACK_HEADER_BYTES 512
    ...
    if (mux->text_comment)
        index_bytes += uint32_t(128 + strlen(mux->text_comment));
...
static float fixedToFloat(int fixed, unsigned precision) {
    return static_cast<float>((double)fixed / (double)(1 << precision));
}
